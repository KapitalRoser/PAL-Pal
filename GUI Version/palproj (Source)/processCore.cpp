...
//implement condensed generation from pal-pal code.
...
int findGap(u32 behind, u32 ahead, bool forward){
  ...
  if (forward){ //what happens when the origin/behind is actually ahead of target/ahead? infinite loop?
    while(behind != ahead){
      LCG(behind);
      counter++;
      if (counter > 100000000){ //This value is usually ok but might break in certain circumstances.
        std::cout <<"Error! findGap() got lost!";
        return -1;
        //Maybe use this as an opportunity to change directions?
      }
    }
